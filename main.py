...
# ── TEMP SMS / ONLINE SMS PH (COMPLETE REBUILD, FIXED) ──

import string, select as sel
...
def update_sms_statuses():
    for key in SMS_SERVICES:
        try:
            r = requests.get(f"https://{key.replace('_','-') if key != 'smsonline' else 'sms-online'}.{'co' if key == 'smsonline' else 'cc' if key == 'receive_sms_online' else 'com'}/", timeout=8)
            SMS_SERVICES[key]["status"] = "available" if r.status_code == 200 else "down"
        except:
...
def parse_phone_numbers(html, patterns=None):
    """Extract phone numbers from HTML using multiple fallback methods."""
    if patterns is None:
        patterns = [
            r'/receive-sms-online/(\+?\d+)',
            r'/number/(\+?\d+)',
            r'/phone/(\+?\d+)',
            r'/numbers/[^/]+/(\+?\d+)',
        ]
...
def get_temp_number():
    ...
    urls = {
        ...
        "receive_sms_online": "https://receive-sms-online.cc/",
        "freephonenum": "https://freephonenum.com/",
        "tempnumber": "https://temp-number.com/numbers/united-states",
    }
    try:
        r = requests.get(urls[svc_key], timeout=15)
        if r.status_code == 200:
            numbers = parse_phone_numbers(r.text)
            if numbers:
                ...
                print(f"\n  {R}No numbers found on the page. The service may be empty.{RES}")
        else:
            print(f"\n  {R}Site returned HTTP {r.status_code}.{RES}")
    except Exception as e:
        print(f"\n  {R}Connection error: {e}{RES}")
    input(f"\n  {DIM}Press ENTER to continue...{RES}")
...
def view_sms_inbox():
    ...
    while True:
        ...
        try:
            ...
            url_templates = {
                "smsonline": f"https://sms-online.co/receive-sms-online/{current_sms_number}",
                "receive_sms_online": f"https://receive-sms-online.cc/number/{clean}/",
                "freephonenum": f"https://freephonenum.com/phone/{clean}/",
                "tempnumber": f"https://temp-number.com/numbers/united-states/{clean}",
            }
            r = requests.get(url_templates.get(svc_key, ''), timeout=10)
            if r.status_code == 200:
                ...
                for row in soup.find_all('tr'):
                    ...
                    if len(cols) >= 3:
                        messages.append({
                            ...
                            "body": cols[1].get_text(strip=True) if len(cols) > 1 else cols[2].get_text(strip=True),
                            "time": cols[-1].get_text(strip=True)
                        })
                if not messages:
                    for div in soup.find_all('div', class_=re.compile(r'message|msg|sms')):
        ...
        if messages:
            for i, msg in enumerate(messages):
                print(f"  {G}[{i+1:02d}]{RES} {msg.get('from','Unknown')}")
                print(f"  {W}    {msg.get('body','')[:100]}{RES}")
                if msg.get('time'): print(f"  {DIM}    {msg['time']}{RES}")
                print()
        else:
...
def copy_sms_number():
    ...
    if not current_sms_number:
        print(f"  {R}No active number.{RES}"); time.sleep(1.5); return
    os.system(f'echo "{current_sms_number}" | termux-clipboard-set 2>/dev/null')
    print(f"  {G}[OK] Copied: {current_sms_number}{RES}"); time.sleep(1.5)
...
def change_sms_service():
    ...
    current_sms_service = None
    sms_session = {}
    print(f"  {G}[OK] Service reset.{RES}"); time.sleep(1)
