...
c(){
    local t="$1"
    local cl=$(echo -e "$t" | sed 's/\x1b\[[0-9;]*m//g')
    local p=$(( (w - ${#cl}) / 2 ))
    [ $p -lt 0 ] && p=0
    printf "%*s%b\n" $p "" "$t"
}
...
s(){
    ...
    while kill -0 $pid 2>/dev/null; do
        local f="${fr:$i:1}"
        local t="Installing ${pkg}..."
        local cl=$(echo -e "$t" | sed 's/\x1b\[[0-9;]*m//g')
        local pad=$(( (w - ${#cl} - 2) / 2 ))
        [ $pad -lt 0 ] && pad=0
        printf "\r%*s%s %s" $pad "" "$f" "$cl"
        i=$(( (i + 1) % 6 ))
        sleep 0.1
    done
    printf "\r%*s✔ %s\n" $pad "" "$cl"
...
# Step 4
{
    curl -sL https://raw.githubusercontent.com/saekacutie/sakea/main/main.py -o $HOME/main.py
...
echo ""
c "${G}${B}✔ All Requirements is now on your Device${R}"
echo ""
